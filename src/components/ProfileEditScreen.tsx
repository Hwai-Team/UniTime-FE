...
export default function ProfileEditScreen({ user, setUser, navigate }: ProfileEditScreenProps) {
  ...
  const handleSave = async () => {
    if (!formData.name || !formData.studentId || !formData.department || !formData.year) {
    ...
    setIsSaving(true);
    try {
      const grade = parseInt(formData.year.replace(/[^0-9]/g, ''), 10) || 0;
      ...
      setUser({
        ...
        year: response.grade ? `${response.grade}학년` : user.year,
        graduationYear: response.graduationYear ? String(response.graduationYear) : user.graduationYear,
      });
          ...
          <Card className="p-8 bg-black/60 backdrop-blur-md border-white/15">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select 
                  value={formData.year} 
                  onValueChange={(value: string) => setFormData({ ...formData, year: value })}
                >
                  <SelectTrigger className="bg-black/40 border-white/20 text-white focus:border-purple-500/50">
              ...
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(value: string) => setFormData({ ...formData, graduationYear: value })}
                >
                  <SelectTrigger className="bg-black/40 border-white/20 text-white focus:border-purple-500/50">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  onClick={handleSave}
                  disabled={isSaving}
                  className="flex-1 gap-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 shadow-[0_0_20px_rgba(140,69,255,0.4)] disabled:opacity-50 disabled:cursor-not-allowed"
                >
