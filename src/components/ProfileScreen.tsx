...
const initialPreviousTimetables: TimetableData[] = [
  ...
  },
  {
    ...
    slots: [
      ...
      { day: '금', time: '10:00', period: '2', subject: '확률과통계', courseCode: 'STAT201', room: '복-410', credits: 3, type: 'general' },
    ],
  },
];
...
export default function ProfileScreen({ user, setUser, navigate, onEditTimetable }: ProfileScreenProps) {
        ...
        setUser(prev => {
          if (!prev) {
            return {
              ...
              graduationYear: profile.graduationYear ? String(profile.graduationYear) : undefined,
          ...
          return {
            ...
            studentId: profile.studentId,
            department: profile.department,
            year: profile.grade ? `${profile.grade}학년` : prev.year,
            graduationYear: profile.graduationYear ? String(profile.graduationYear) : prev.graduationYear,
  ...
  const calculateCredits = (slots: TimeSlot[]) => {
    const majorCredits = slots
      .filter(slot => slot.type === 'major')
      .reduce((sum, slot) => sum + (slot.credits || 0), 0);
    
    const generalCredits = slots
            ...
            <TabsContent value="previous" className="space-y-4">
              ...
              {previousTimetables.length === 0 ? (
                <div className="text-center text-white/40 py-20">
                  <p>등록된 시간표가 없습니다.</p>
                  <p className="text-sm mt-2">위의 "시간표 추가" 버튼을 눌러 시작하세요.</p>
                </div>
              ) : (
